...
pub fn init() {
    ...
    conn.execute_batch("ALTER TABLE debates ADD COLUMN infoslide TEXT NOT NULL DEFAULT '';")
        .ok();
...
pub fn get_debates() -> Vec<(String, String)> {
    with_db(|conn| {
        let mut stmt = conn
            .prepare("SELECT id, motion FROM debates ORDER BY rowid")
            .unwrap();
...
pub fn create_debate(id: &str, motion: &str, infoslide: &str) {
    with_db(|conn| {
        conn.execute(
            "INSERT INTO debates (id, motion, infoslide) VALUES (?1, ?2, ?3)",
            params![id, motion, infoslide],
        )
        .unwrap();
...
pub fn upsert_debate(id: &str, motion: &str, infoslide: &str) {
    with_db(|conn| {
        conn.execute(
            "INSERT OR REPLACE INTO debates (id, motion, infoslide) VALUES (?1, ?2, ?3)",
            params![id, motion, infoslide],
        )
        .unwrap();
        for role in ["PM", "LO", "DPM", "DLO", "MG", "MO", "GW", "OW"] {
...
pub fn delete_debate(id: &str) {
    with_db(|conn| {
        conn.execute("DELETE FROM debates WHERE id=?1", params![id])
            .unwrap();
    });
}
...
pub fn get_debate(id: &str) -> Option<Debate> {
    with_db(|conn| {
        let (motion, infoslide): (String, String) = conn
            .query_row(
                "SELECT motion, infoslide FROM debates WHERE id=?1",
                params![id],
        ...
        let mut stmt = conn
            .prepare("SELECT role, speaker, speech, rebuttal, poi FROM speeches WHERE debate_id=?1")
            .unwrap();
        ...
        .unwrap()
        .filter_map(|r| r.ok())
        .for_each(|(role, speaker, speech, rebuttal, poi)| {
            let s = Speech {
                speaker,
                speech,
                rebuttal,
